...
@section('content')
    <div class="px-4 sm:px-6 lg:px-8">
        <form action="{{ route('admin.contentdescriptions.update', $contentdescription) }}" method="POST"
            enctype="multipart/form-data" class="bg-white shadow-xs ring-1 ring-gray-900/5 sm:rounded-xl my-4 md:my-8">
            @method('PUT')
            @csrf
                ...
                <div class="space-y-12">
                    <div class="border-b border-gray-900/10 pb-3">
                        <div class="text-base/7 font-semibold text-gray-900">
                            {{ trans('global.edit') }} {{ __('labels.contentdescription.title_singular') }}
                        </div>
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <div class="mt-2">
                                    <select name="section_id" id="section_id" required
                                        class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus:outline-1 focus:-outline-offset-1 focus:outline-blue sm:text-sm/6">
                                        <option value="">Select Section</option>
                                        @foreach ($sections as $section)
                                            <option value="{{ $section->id }}"
                                                {{ old('section_id', $contentdescription->section_id) == $section->id ? 'selected' : '' }}>
                            ...
                            <div class="sm:col-span-3">
                                ...
                                <div class="mt-2">
                                    <input type="text" name="title" id="title"
                                        value="{{ old('title', $contentdescription->title) }}" required
                                        class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-1 focus:-outline-offset-1 focus:outline-blue sm:text-sm/6">
                                </div>
                                @if ($errors->has('title'))
                                    @include('admin.common.validation-error', [
                                        'field' => 'title',
                            ...
                            <div class="sm:col-span-6">
                                ...
                                <div class="mt-2">
                                    <textarea name="description" id="description" rows="4"
                                        class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-1 focus:-outline-offset-1 focus:outline-blue sm:text-sm/6">{{ old('description', $contentdescription->description) }}</textarea>
                                </div>
                            ...
                            <div class="sm:col-span-3">
                                <div class="">
                                    <input type="hidden" name="key" id="key"
                                        value="{{ old('key', $contentdescription->key) }}" required
                                        class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-1 focus:-outline-offset-1 focus:outline-blue sm:text-sm/6">
                                </div>
                                @if ($errors->has('key'))
                                    @include('admin.common.validation-error', [
...
@section('scripts')
    ...
    <script>
        ...
        Dropzone.options.featuredImageDropzone = {
            ...
            init: function() {
                @if (isset($contentdescription) &&
                        $contentdescription->getMedia('featured_image')->first() &&
                        $contentdescription->getMedia('featured_image')->first() !== '/user-avatar.png')
                    var fileName = {!! json_encode(\App\Models\ContentDescription::getImageName($contentdescription->featured_image)) !!};
                    ...
                    var publicUrl = '';
                    var imgPath = {!! json_encode($contentdescription->getMedia('featured_image')->first()->getFullUrl()) !!};
